...
public class FriendsUserServiceImpl extends ServiceImpl<FriendsUserMapper, FriendsUser> implements FriendsUserService {
    ...
    public Result add(String formUserId, String toUserId) {
        ...
        if (friendsUser != null) {
            ...
            if (status.equals("1")) {
                return Result.fail(400, "申请失败,已是你的好友了");
            }
            if (status.equals("0")) {
                return Result.fail(400, "申请失败,已申请,对方未回应");
            }
            if (status.equals("-1")) {
                friendsUser.setStatus("0");
                int i = friendsUserMapper.updateById(friendsUser);
                if (i == 0) {
                    return Result.fail(500, "服务器出现错误,请反馈给问题");
                }
    ...
    @Override
    public Result getListFriendsUserRefuse(String userId) {
        List<FriendsUser> listFriendsUserByUserId = getListFriendsUserByUserId(userId, false,"-2");
        List<FriendsUser> listFriendsUserByUserId2 = getListFriendsUserByUserId(userId, true, "-2");
        listFriendsUserByUserId.addAll(listFriendsUserByUserId2);
        List<FriendsUsersAndUserVo> friendsUsersAndUserVos = copyFriendsUserList(listFriendsUserByUserId);
        return Result.succ("获取拒绝好友通知列表成功", friendsUsersAndUserVos);
    }

    //同意申请好友、单方面删除好友、拒绝申请好友
    @Override
    public Result updateStatus(String formUserId, String toUserId, String status) {
        ...
        if (status.equals("-2")) {
            ...
            FriendsUser toUser = getMethod(toUserId, formUserId);
            if (toUser == null) {
                return Result.succ("拒绝成功");
            }
            toUser.setStatus(status);
            friendsUserMapper.updateById(toUser);
            return Result.succ("拒绝成功");
        }
        return Result.fail(400, "status参数错误");
    }
    ...
    public FriendsUser getMethod(String formUserId, String toUserId) {
        ...
        friendsUserLambdaQueryWrapper.eq(FriendsUser::getFormUserId, formUserId)
                .eq(FriendsUser::getToUserId, toUserId);
        return friendsUserMapper.selectOne(friendsUserLambdaQueryWrapper);
    }
